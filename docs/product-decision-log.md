...
This document records the main product decisions behind the MVP. Each decision protects analytical credibility or keeps the prototype focused enough for a clear demonstration.
...
**Rationale:** English reflects common SaaS analytics terminology and makes the project accessible in an international portfolio. Chinese localization makes the same prototype usable in domestic AI product interviews and demonstrates that localization includes analytical content, not only navigation labels.
...
**Rationale:** An open-ended model can produce plausible but unsupported conclusions, change its reasoning between runs or confuse metric denominators. The rule engine establishes a stable factual layer. AI can explain that layer but cannot overwrite it.
...
**Trade-off:** Rule coverage is narrower and must be maintained explicitly, but the result is easier to test and defend.
...
**Implementation boundary:** Each hypothesis must include the observed evidence and state what additional event, exposure or experiment data would validate it.
...
**Rationale:** A fixed strategy list would make Business Setup decorative and the Priority Matrix untrustworthy. Transparent scoring shows why one intervention should be tested before another and allows an interviewer or stakeholder to challenge the assumptions.
...
## 7. Do not expand additional industry templates yet
...
**Rationale:** Replacing labels while reusing fitness assumptions would create superficial localization by industry and weaken credibility. Each vertical needs a defensible event model, funnel, benchmarks and strategy library.
...
**Trade-off:** The product has a narrower addressable demo today, but future templates can be added as genuine domain modules instead of cosmetic variants.
